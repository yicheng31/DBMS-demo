...
1. Added get_user_profile and get_payment_info tools
1. Human-friendly system prompt and error messages
1. Booking confirmation with context recovery from history
1. Structured, emoji-enhanced response formatting
1. Stronger fallback: overrides wrong tool selections
1. Greeting protection: skip tool calls for simple greetings
1. Chinese policy query translation for vector search
1. Pre-classification: categorize query BEFORE LLM (14→2-4 tools)
1. Automatic date extraction from natural language
1. Multi-step chaining: booking queries auto-call availability+fare+seats
1. Cancel vs policy smart classification
1. Pre-login check: prompt login BEFORE running booking chain
1. Ticket type extraction (single/return)
1. Seat preference extraction (window/aisle)
1. Multi-schedule selection: list options for user to choose
1. Stronger confirmation message format in SYSTEM_PROMPT
1. Station ID deduplication (BUG FIX #1)
1. Booking context recovery uses correct search order (BUG FIX #2)
1. Fare class extracted from USER messages only (BUG FIX #3)
1. Continuation dialog detection from history (BUG FIX #4)
1. hops=0 support: extract from message, None check prevents 0→2 (BUG FIX #5)
1. Avoid keyword detection → find_alternative_routes (BUG FIX #6)
1. Cross-network alternative routes force network=auto (BUG FIX #7)
   “””
...
if any(kw in lower for kw in delay_kw):
    return "delay"
if any(kw in lower for kw in connections_kw) and _station_ids:
    return "connections"
